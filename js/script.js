const loadMobileName = () => {
    ...
    searchContent.value = "";
    if (searchText == "") {
        alert("Please give some food name");
    } else {
...
const displayMobileName = (mobiles) => {
    ...
    if (mobiles.length == 0) {
        ...
        hiddenDiv.innerHTML = `
            <h3 class="text-center"> Please give a correct phone name</h3> 
        `;
        displayMobiles.appendChild(hiddenDiv);
    } else {
...
const displayMobileData = (mobileData) => {
    ...
    div.innerHTML = `
        ...
        <div class="card-body p-4">
            ...
            <p class="lead"><b>Sensors:</b> ${mobileData.mainFeatures.sensors.join(
                ", "
            )}</p>
        </div>
    `;
